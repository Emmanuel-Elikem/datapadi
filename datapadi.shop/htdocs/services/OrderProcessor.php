...
class OrderProcessor {
    private $db;
    private $api;
    private $manualWindowMinutes = 1; // Changed from 10 to 1 minute for faster processing
    ...
    public function processOrder($orderData) {
        // If this payment_ref already exists, return existing order_id to avoid duplicates
        if (!empty($orderData['payment_ref'])) {
            $stmt = $this->db->prepare("SELECT order_id FROM orders WHERE payment_ref = ? LIMIT 1");
            $stmt->execute([$orderData['payment_ref']]);
...
public function processAutomatically($orderId) {
    ...
    public function checkAndUpdateStatus($orderId) {
        try {
            ...
            if (!$order) {
                return ['success' => false, 'message' => 'Order not found or not processing via API'];
            }
            
            // Check status via API
            $statusData = $this->api->checkStatus($order['provider_ref']);
            
            if (!$statusData) {
                return ['success' => false, 'message' => 'Order not found on provider'];
            }
            
            // Map provider status to local status
            // Provider statuses: pending, processing, completed, failed, refunded
            $providerStatus = strtolower($statusData['status']);
            $newStatus = $order['status']; // Default to current
            
            if ($providerStatus === 'completed' || $providerStatus === 'success') {
                $newStatus = 'completed';
            } elseif ($providerStatus === 'failed' || $providerStatus === 'refunded' || $providerStatus === 'cancelled') {
                $newStatus = 'failed';
            }
            ...
            if ($newStatus !== $order['status']) {
                ...
                $note = "Status updated from API: " . ucfirst($providerStatus);
                if (isset($statusData['message'])) {
                    $note .= " - " . $statusData['message'];
                }
                
                $this->addStatusHistory($orderId, $newStatus, $note);
            ...
            return ['success' => true, 'message' => 'Status unchanged (' . $providerStatus . ')'];
